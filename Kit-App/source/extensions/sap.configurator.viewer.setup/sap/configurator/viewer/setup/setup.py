...
ICON_PATH = carb.tokens.get_tokens_interface().resolve("${sap.configurator.viewer.setup}/data/icons")
...
class SetupExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id: str) -> None:
        ...
        # from Configurator autoload functionality ---
        self._stage_url = self._settings.get_as_string("/app/auto_load_usd")    # get auto load stage name
        # print(f"***** sap.configurator.viewer setup.py setting /app/auto_load_usd={self._stage_url}")
        if self._stage_url:
    ...
    def _setup_ui_state_changes(self) -> None:
        ...
        for window in window_titles:
            setting_name = f'/exts/sap.configurator.viewer.setup/{window}/visible'
        ...
        self._ui_state_manager.add_settings_copy_dependency(
            source_path="/exts/sap.configurator.viewer.setup/Markups/visible",
            target_path="/exts/omni.kit.markup.core/show_icons",
        )

        self._ui_state_manager.add_settings_copy_dependency(
            source_path="/exts/sap.configurator.viewer.setup/Waypoints/visible",
    ...
    def _custom_quicklayout_menu(self) -> None:
        ...
        def add_layout_menu_entry(name, parameter, key):
            ...
            layouts_path = carb.tokens.get_tokens_interface().resolve("${sap.configurator.viewer.setup}/layouts")
    ...
    # from Configurator  (to autoload USD file or url)
    async def __open_stage(self, url):
        # print(f"*****sap configurator viewer:__open_stage url={url}")
        # 10 frame delay to allow Layout
        for i in range(5):
            await omni.kit.app.get_app().next_update_async()  # type: ignore

        usd_context = omni.usd.get_context()
        print(f"***** sap configurator viewer setup.py open_stage_async: url={url}")
